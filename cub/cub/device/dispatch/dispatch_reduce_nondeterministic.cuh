  ...
  KernelLauncherFactory launcher_factory = {})
{
  ...
  const ReduceNondeterministicPolicy active_policy = policy_selector(cc);
  ...
  const int tile_size = active_policy.multi_tile.threads_per_block * active_policy.multi_tile.items_per_thread;
  int sm_occupancy;
  if (const auto error = CubDebug(launcher_factory.MaxSmOccupancy(
        sm_occupancy, kernel_source.NondeterministicAtomicKernel(), active_policy.multi_tile.threads_per_block)))
  {
    return error;
  }
  int sm_count;
