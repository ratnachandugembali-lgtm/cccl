...
{
  constexpr ReduceDeterministicPassPolicy policy = current_policy<PolicySelector>().multi_tile;
...
_CCCL_KERNEL_ATTRIBUTES __launch_bounds__(
  ...
  1) void DeterministicDeviceReduceSingleTileKernel(InputIteratorT d_in,
                                                    ...
                                                    TransformOpT transform_op)
{
  constexpr ReduceDeterministicPassPolicy policy = current_policy<PolicySelector>().single_tile;
  constexpr int threads_per_block                = policy.threads_per_block;
