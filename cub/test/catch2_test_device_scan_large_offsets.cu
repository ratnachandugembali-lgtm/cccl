...
C2H_TEST("DeviceScan exclusive scan works for very large number of items", "[large-mem][scan][device]", offset_types)
try
{
...
C2H_TEST("DeviceScan inclusive scan works for very large number of items", "[large-mem][scan][device]", offset_types)
try
{
  using op_t     = cuda::std::plus<>;
  using item_t   = int;
